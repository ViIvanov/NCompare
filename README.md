# NCompare
Library providing type `ComparerBuilder<T>` that helps to build comparators (equality comparators and sort comparators) for compare objects of type `T`.

`ComparerBuilder<T>` helps to compare a complex types. It can build an `EqualityComparer<>` and/or a `Comparer<>` objects for your types.
You just need to "add" expressions, that describes what data do you want to compare and, optionally, comparators - to specify how to compare that data.

You can find reasons for making `ComparerBuilder<>` in this old post on [RSDN](http://rsdn.ru/forum/src/3914421.1) (in Russian).

Now `ComparerBuilder<T>` written on C# 11.0. You can compose a comparer builder
from other comparer builders. Also, it supports "intercepted" comparators, that allows
to intercept compare methods and, for example, print to a `Debug` when `IEqualityComparer<>::Equals(…, …)` returns `false` or `IComparer<>::Compare(…)` returns not `0`.
"Intercepted" comparators are helpful in a debugging scenarios.
